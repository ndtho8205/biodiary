...
public class CvCameraPreview extends SurfaceView implements SurfaceHolder.Callback, PreviewCallback
{
    ...
    public CvCameraPreview(Context context, AttributeSet attrs)
    {
        ...
        TypedArray array = getContext().obtainStyledAttributes(attrs,
                                                               R.styleable.CvCameraPreview);
        int camType   = array.getInt(R.styleable.CvCameraPreview_camera_type, CAMERA_FRONT);
        int scaleType = array.getInt(R.styleable.CvCameraPreview_scale_type, SCALE_FIT);
        array.recycle();

        mWindowManager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);

        initializer(camType == CAMERA_BACK
                    ? Camera.CameraInfo.CAMERA_FACING_BACK
                    : Camera.CameraInfo.CAMERA_FACING_FRONT, scaleType);
    }
            ...
            if (Build.VERSION.SDK_INT >= 9) {
                ...
                if (this.cameraId < 0) {
                    ...
                    for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
                        ...
                        if (camInfo.facing == cameraType) {
                            ...
                            try {
                                ...
                                Log.e(LOG_TAG,
                                      "initializeCamera(): trying to open camera #" + i +
                                      " but it's locked",
                                      e);
                            }
                        }
                    ...
                    try {
                        ...
                        Log.e(LOG_TAG,
                              "initializeCamera(): trying to re-open camera #" + this.cameraId +
                              " but it's locked",
                              e);
                    }
                }
    ...
    private boolean setupCamera()
    {
        ...
        try {
            ...
            if (sizes != null) {
                ...
                try {
                    this.previewBuffer = new byte[prevWidth * prevHeight *
                                                  ImageFormat.getBitsPerPixel(cameraDevice.getParameters()
                                                                                          .getPreviewFormat()) /
                                                  8];
                    setCameraCallback();
                }
    ...
    private Size getBestSize(List<Size> sizes, int widthThreshold)
    {
        ...
        for (Size currentSize : sizes) {
            boolean isDesiredRatio = ((currentSize.width / ASPECT_RATIO_W) ==
                                      (currentSize.height / ASPECT_RATIO_H));
        ...
        if (bestSize == null) {
            ...
            Log.e(LOG_TAG,
                  "determineBestSize(): can't find a good size. Setting to the very first...");
        }

        Log.i(LOG_TAG,
              "determineBestSize(): bestSize is " + bestSize.width + "x" + bestSize.height);
        return bestSize;
    }
...
//            filter.stop();
            ...
            Log.i(LOG_TAG,
                  "stopCameraPreview(): tried to stop a non-running preview, this is not an error");
        }
    }
    ...
    private void initFilter(int width, int height)
    {
        ...
        Log.i(LOG_TAG,
              "init filter with width = " + width + " and height = " + height + " and degree = "
